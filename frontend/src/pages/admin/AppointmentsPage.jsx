...
const AppointmentsPage = () => {
	...
	const fetchAppointments = async () => {
		try {
			setLoading(true);
			// Luôn lấy tất cả, không gửi params status
	...
	const filteredAppointments = appointments.filter(app => {
		const matchesSearch = (app?.viewer_name?.toLowerCase() || "").includes(searchTerm.toLowerCase()) ||
	...
	return (
		<Box sx={{ p: { xs: 2, md: 3 }, bgcolor: "background.default", minHeight: "100%" }}>
			...
			{/* Filters card */}
			<Card sx={{ p: 2, mb: 3, borderRadius: 3, boxShadow: "0 4px 20px rgba(0,0,0,0.05)", border: "1px solid", borderColor: "divider" }}>
			...
			{/* Main Table */}
			<TableContainer component={Paper} sx={{ borderRadius: 3, boxShadow: "0 10px 40px rgba(0,0,0,0.03)", border: "1px solid", borderColor: "divider", overflow: "hidden" }}>
				{loading ? (
