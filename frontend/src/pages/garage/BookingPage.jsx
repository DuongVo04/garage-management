...
import {
  Person, Phone, Email, Notes, CheckCircle, Build,
...
const BookingPage = () => {
  ...
  // ── Seed selected IDs: router state (Garage) → URL param → rỗng ──────────
  useEffect(() => {
    const stateIds = location.state?.selectedIds;
    if (stateIds?.length > 0) {
      setSelectedServiceIds(stateIds);
    } else if (serviceId) {
  ...
  const isFormValid = useMemo(() =>
    selectedServiceIds.length > 0 && Object.values(fieldErrors).every(e => e === ''),
    [fieldErrors, selectedServiceIds]);
  ...
  }), [fieldErrors, formData, selectedServiceIds]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        customer_id: user?.id, // Backend sẽ xử lý đây là account_id hoặc customer_id
        customer_info: {
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <Paper elevation={0} sx={{ p: 3.5, borderRadius: '16px', border: `1px solid ${TOKEN.border}`, bgcolor: TOKEN.white }}>
                <Grid container spacing={2.5}>
                  ...
                  <Grid item xs={12}>
                    <TextField fullWidth label="Ghi chú (tuỳ chọn)" name="note"
                      multiline rows={3}
                      placeholder="Mô tả tình trạng xe hoặc yêu cầu đặc biệt..."
                      value={formData.note} onChange={handleChange}
                      sx={{
                        '& .MuiOutlinedInput-root': {
                          borderRadius: '10px', bgcolor: TOKEN.white, fontSize: '0.9rem',
                          '& fieldset': { borderColor: TOKEN.border },
                          '&:hover fieldset': { borderColor: TOKEN.coal },
                          '&.Mui-focused fieldset': { borderColor: TOKEN.coal, borderWidth: 2 },
                        },
                      ...
                      InputProps={{
                        startAdornment: <Notes sx={{ mr: 1, fontSize: 18, color: TOKEN.muted, alignSelf: 'flex-start', mt: 1.5 }} />
                      }}
                    />
                  </Grid>
                </Grid>
                  ...
                  <>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', mb: 3 }}>
                      <Typography variant="body2" fontWeight={700} sx={{ color: TOKEN.muted }}>
                        Tổng cộng ({selectedServices.length})
                      </Typography>
                  ...
                  <Box sx={{ mb: 2 }}>
                    {selectedServiceIds.length === 0 && (
                      <Box sx={{ display: 'flex', gap: 0.8, alignItems: 'center', mb: 0.5 }}>
                        ...
                        <Typography variant="caption" sx={{ color: TOKEN.muted }}>Chọn ít nhất một dịch vụ</Typography>
                      </Box>
                    )}
          ...
          <Box sx={{ p: 4, textAlign: 'center' }}>
            ...
            <Box sx={{ bgcolor: TOKEN.surface, borderRadius: '12px', p: 2.5, mb: 3, textAlign: 'left' }}>
              {selectedServices.slice(0, 3).map(s => (
                <Box key={s.id} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="caption" fontWeight={600} sx={{ color: TOKEN.slate }}>{s.name}</Typography>
                  <Typography variant="caption" fontWeight={700}>{formatCurrency(s.price)}</Typography>
                </Box>
              ))}
              {selectedServices.length > 3 && (
                <Typography variant="caption" sx={{ color: TOKEN.muted }}>
                  +{selectedServices.length - 3} dịch vụ khác
                </Typography>
              )}
              {formData.appointment_date && (
